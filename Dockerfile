...
ADD settings/supervisor/kafka.conf settings/supervisor/zookeeper.conf settings/supervisor/create-topics.conf /etc/supervisor/conf.d/
ADD settings/kafka/server.properties settings/kafka/zookeeper.properties /opt/kafka_2.12-2.6.0/config/
ADD settings/kafka/topic-list /root/
ADD settings/kafka/scripts/create-topics.sh /root/
RUN chmod +x /root/create-topics.sh
